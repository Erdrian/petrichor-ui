import { useState, forwardRef, useCallback, ReactNode, useImperativeHandle, useMemo } from 'react'
...
import useLoginEffect from '../hook/useLoginEffect'
...
export type filterInHistory = historyState | undefined
...
export default forwardRef<any, EnquiryFormProps>((props, ref) => {
	...
	// 表格数据获取方法
	const getData = async (props: { filter: object; page: number; pageSize: number }) => {
		setIsloading(true)
	...
	useImperativeHandle(ref, () => ({
		getData: (arg: { filter: object; page: number; pageSize: number }) => {
			let _filter = arg?.filter || filter
			let _page = arg?.page || page
			let _pageSize = arg?.pageSize || pageSize
			getData({ filter: _filter, page: _page, pageSize: _pageSize })
		},
		data: dataSource,
	}))
	// 搜素时的回调函数
	const onSearch = useCallback((filter: object) => {
		setPage(1)
		setFilter(filter)
